...
(async () => {
  ...
  //   const products = await articlesIntoProducts(productshtml);
  console.log(productshtml);
  // single product info
  console.log(await productshtml[0].evaluate((ele) => ele.getAttribute("href")));
})();

async function ExtractProductInfo(productCard) {
  let mehref = await productCard.evaluate((ele) => ele.getAttribute("href"));
  let tittle = await productCard.evaluate(
    (ele) => ele.querySelector(".plpTitle").innerText
  );
  let price = await productCard.evaluate((ele) =>
    ele.querySelector(".price-wrapper").getAttribute("data-price-amount")
  );
